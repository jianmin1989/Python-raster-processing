...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 16:09:59 2019

@author: wangj
"""


Pathref = '/hunter/data1/wangj/LSP_VIIMOD/MCD43A4/'
Pathqa = '/hunter/data1/wangj/LSP_VIIMOD/MCD43A2/'
Pathref = '/gpfs/data/xyz/jianmin/westUS/MCD43A4/'
Pathqa = '/gpfs/data/xyz/jianmin/westUS/MCD43A2/'
Pathout = '/gpfs/data/xyz/jianmin/westUS/composite3d_ndwi/'
Tiles = ['h09v05'] ## ['h10v05']  
...
#                    outfile = pathout + 'VIIRS_VI.'+dates[0]+'.' + tile + '.BIP.gz'
                outfile = '%sMODIS_VI.%s.%s.BIP.gz' % (pathout, dates[0], tile)
                ...
                files_ref = ["%sMCD43A4.A%s.%s.006.hdf" % (pathref, item, tile) for item in dates ]
                files_qa = ["%sMCD43A2.A%s.%s.006.hdf" % (pathqa, item, tile) for item in dates ]
        ...
        if outstyle == 1:
            ...
            for year in years:
                ...
                outfile = '%sMODIS_VI.%s.%s.year%s.BIP' % (pathout, year, tile, nyear)
                ...
                files=['%sMODIS_VI.%s%s.%s.BIP.gz' % (pathout, yy, str(dd).zfill(3), tile) for yy, dd in zip(yys, dds)]
...
def index_calculate(REF_files, QA_files) : 
    ...
    [print("%s is missed" % file) for file in QA_files if not os.path.isfile(file)]
    ...
    if n > 0:  
        ...
        for k in range(n):
            ...
            ref_I1 = hand_ref.select('Nadir_Reflectance_Band1').get() #.size and np.shape() to check the size
            ref_I2 = hand_ref.select('Nadir_Reflectance_Band2').get()
            ref_I3 = hand_ref.select('Nadir_Reflectance_Band6').get() ### SWIR or I3 in VIIRS 
            qa_I1 = hand_ref.select('BRDF_Albedo_Band_Mandatory_Quality_Band1').get() #.size and np.shape() to check the size
            qa_I2 = hand_ref.select('BRDF_Albedo_Band_Mandatory_Quality_Band2').get() 
            #qa_I3 = hand_ref.select('BRDF_Albedo_Band_Mandatory_Quality_Band6').get() 
            hand_ref.end()
            
            hand_qa = SD(file_qa, SDC.READ)
            snow = hand_qa.select('Snow_BRDF_Albedo').get()
            water = hand_qa.select('BRDF_Albedo_LandWaterType').get()
            hand_qa.end()
            ...
            qa[water !=1] = 100 ### water
            qa[snow == 1]=1
            qa[(evi2 == FILL) | (ndvi==FILL) | (qa_I1 >=4) | (qa_I2 >= 4)] = 9 ##FILL
